...
- src/ops.rs - Defines the 33 ksplang instructions
- src/compiler/
  ...
  - osmibytecode.rs - defines osmibytecode (OBC) opcodes
  - osmibytecode_backend.rs - compiles CFG to osmibytecode
  - osmibytecode_vm.rs - OBC interpreter
...
- We perform on-the-fly optimizations, there are currently no passes
...
- errors are (for now) always interpreted as deopt
  - we let the reference interpreter handle failures if they happen
- precompiler has a local symbolic stack of ValueIds, which will be a small subset of the execution stack at runtime
...
We have fuzz tests verifying that JIT behaves the same as reference interpreter. It's run with `cargo +nightly fuzz run fuzz_target_2`, agent should not start this.

We make unit tests from fuzz crashes, in fuzz/tests/regression_tests.rs.
First named fuzz_repro, then renamed based on the bug found.
...
- Do not bother with assert/panic error messages, we'll add it when it fails
...
- `SmallVec` and `ArrayVec` for small, inline-allocated collections in hot paths
- Do not bother with API backwards compatibility, this is not a library

## Debugging Tips

- println! + dbg! and search
- prefix DBG for temp logs, i.e.: `println!("DBG1 {a} {b}`)`
- don't bother with log level for temporary prints
- Use assert!
- CFG, Osmibytecode and other structs implement `Display` for readable IR dumps
- most fuzz_repro can be fixed very simply, but it may be hard to find the bug
...
Don't run git commands which change the .git folder (staging, commiting, ...).
Repo may contain related or unrelated staged or unstaged changes, please let them be.
